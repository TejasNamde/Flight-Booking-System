  ...
  if($stop=="nonstop"){
    ...
    while($row = pg_fetch_array($result)) {
        ...
        echo "<td>" . $row['price'] . "</td>";
            ...
            $reserved = pg_query($con, $seatreserved);   
            $reservedNumber = pg_fetch_array($reserved);
            
            $capacity = pg_query($con, "SELECT capacity FROM class C WHERE C.number='".$row['FLnumber']."' AND C.name= '".$row['classname']."'");
        ...
        echo "<td>" . $row4['price'] . "</td>";
         ...
         //calculate number of remain seats
            $seatreserved4 = "SELECT flightno, classtype, COUNT(*)
                        ...
                        GROUP BY flightno, classtype";
            $reserved4 = pg_query($con, $seatreserved4);   
            $reservedNumber4 = pg_fetch_array($reserved4);
            
            $capacity4 = pg_query($con, "SELECT capacity FROM class C WHERE C.number='".$row4['FLnumber']."' AND C.name= '".$row4['classname']."'");
